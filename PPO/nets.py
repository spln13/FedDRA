...
class Stage2ActorClientwise(nn.Module):
    ...
    def forward(self, S2_cli: torch.Tensor) -> torch.Tensor:
        # 支持 [N, d] 或 [B, N, d]
        if S2_cli.dim() == 2:
            logits = self.mlp(S2_cli).squeeze(-1)  # [N]
            return logits.unsqueeze(0)  # [1, N]
        if S2_cli.dim() == 3:
            logits = self.mlp(S2_cli).squeeze(-1)  # [B, N]
            return logits
        raise ValueError(f"Stage2ActorClientwise expects 2D/3D input, got shape {tuple(S2_cli.shape)}")
